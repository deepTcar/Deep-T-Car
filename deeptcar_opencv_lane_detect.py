...
_SHOW_IMAGE = False
class DTCOpencvLaneDetect(object):
...
def detect_edges_old(frame):
    ...
    for i in range(16):
        ...
        upper_blue = np.array([150, 255, 255])
        mask = cv2.inRange(hsv, lower_blue, upper_blue)
        show_image("Bed mask Sat=%s" % (16* i), mask)
...
def compute_steering_angle(frame, lane_lines):
    ...
    if len(lane_lines) == 1:
        ...
        camera_mid_offset_percent = 0.15 # 0.0 means car pointing to center, -0.03: car is centered to left, +0.03 means car pointing to right
